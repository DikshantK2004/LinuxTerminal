...
bool latestChecker(struct stat a ,struct stat b)
{
    ...
    long long int time2 = b.st_mtime;
...
void CpMv(char parsedInput[MAX][MAX] , int args)
{
    ...
    for(int i = 2 ;i < args + 1 ; i++)
    {
        ...
        long long int mode;
        FILE* this;
        if(strlen(current.dir_name) == 0) this = fopen(current.file_name , "rb");
        else if(chdir(current.dir_name) != -1) this = fopen(current.file_name , "rb");
        else 
        {
        ...
        if(chdir(parsedInput[args + 2 -1]) != -1)
        {
            ...
            memset(dump , '\0' ,MAX);
            getcwd(dump , MAX);
            if(!implementFlags(current.file_name)) continue;
